...
extern NSString *const kAHawkeyeCalltraceAutoLaunchKey; /**< standardUserDefaults key, 启动时是否在 load 自动开启 CallTrace */


@class AMTHCallTraceTimeCostModel;
 ...
 */
@interface AMTHCallTrace : NSObject
...
+ (NSArray<AMTHCallTraceTimeCostModel *> *)records;                           // 原始排列的数据
+ (NSArray<AMTHCallTraceTimeCostModel *> *)prettyRecords;                     // 原始排列的数据，将子节点移为树枝
+ (NSArray<AMTHCallTraceTimeCostModel *> *)recordsFromIndex:(NSInteger)index; // 原始的排列数据，从偏移位置开始到结尾
