...
static amth_call_record *_callrecords;
...
static inline uintptr_t pop_call_record() {
    ...
    if (cs->is_main_thread && _call_record_enabled) {
        ...
        if (_trace_all || (cost > _record_time_threshold && cs->index < _max_call_depth)) {
            if (!_callrecords) {
                _recordAlloc = 1024;
                _callrecords = malloc(sizeof(amth_call_record) * _recordAlloc);
            }
            _callrecordNum++;
            if (_callrecordNum >= _recordAlloc) {
                _recordAlloc += 1024;
                _callrecords = realloc(_callrecords, sizeof(amth_call_record) * _recordAlloc);
            }
            amth_call_record *log = &_callrecords[_callrecordNum - 1];
            log->cls = pRecord->cls;
            log->depth = curIndex;
            log->sel = pRecord->cmd;
            log->cost = cost;
...
void amth_calltraceStop(void) {
    _call_record_enabled = false;
}

bool amth_calltraceRunning(void) {
    return _call_record_enabled;
}

void amth_calltraceConfigTimeThreshold(uint32_t us) {
    _record_time_threshold = us;
}

void amth_calltraceConfigMaxDepth(int depth) {
    _max_call_depth = depth;
}

void amth_calltraceTraceAll(void) {
    _trace_all = true;
}

void amth_calltraceTraceByThreshold(void) {
    _trace_all = false;
}

uint32_t amth_calltraceTimeThreshold(void) {
    return _record_time_threshold;
}

int amth_calltraceMaxDepth(void) {
    return _max_call_depth;
}

amth_call_record *amth_getCallRecords(int *num) {
    if (num) {
        *num = _callrecordNum;
    }
    return _callrecords;
...
void amth_clearCallRecords(void) {
    if (_callrecords) {
        free(_callrecords);
        _callrecords = NULL;
    }
...
void amth_calltraceDisableTemp() {}
void amth_calltraceEnableTemp() {}
